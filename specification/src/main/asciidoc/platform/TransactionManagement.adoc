...
when this may be needed include situations with changed security attributes,
isolation levels, character settings, and localization configuration. Containers
must not attempt to share connections that are marked unshareable. If a
connection is not marked unshareable, it must be transparent to the application
whether the connection is actually shared or not.
...
is provided. link:#a3399[See Jakarta EE Application Client XML Schema]”, the
Jakarta Enterprise Beans specification, and the Servlet specification provide
...
The JDBC transaction requirements in link:#a516[See Transactional JDBC™
Technology Support]” and the Jakarta Messaging transaction requirements in
link:#a520[See Transactional Jakarta Messaging Support]” may impose some
...
XA-capable resource adapters. See also link:#a516[See Transactional JDBC™
Technology Support].”
