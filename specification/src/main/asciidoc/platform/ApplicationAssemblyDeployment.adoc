...
and defines four modules with default names "enterprisebeans/inventory",
"enterprisebeans/accounts", "ui/store", and "ui/admin".
...
classes that are shared between enterprise beans in two separate enterprisebean-jar
files.
...
app1.ear:
 ...
 enterprisebean1.jar Class-Path: util.jar

 enterprisebean2.jar Class-Path: util.jar
...
those classes. Those classes are needed both by _enterprisebean2.jar_ , packaged in
the same application as _enterprisebean1.jar_ , and by _enterprisebean3.jar_ and
_jakartaservlet1.jar_ , packaged in a different application. Those classes are
also needed by _enterprisebean1.jar_ itself because they define the remote
interface of the enterprise beans in _enterprisebean1.jar_ , and the developer has
chosen the by reference model of making these classes available, as
described in the Jakarta Enterprise Beans spec. The deployment descriptor for _enterprisebean1.jar_
names the client view JAR file in the _enterprisebean-client-jar_ element.

The _Class-Path_ mechanism must be used by
components in _app3.ear_ to reference the client view JAR file that
corresponds to the enterprise beans packaged in _enterprisebean1.jar_ of _app2.ear_
. These enterprise beans are referenced by enterprise beans in
_enterprisebean3.jar_ and by the Jakarta Servlets packaged in _webapp.war_ .
...
app2.ear:
 ...
 enterprisebean1.jar Class-Path: enterprisebean1_client.jar

 deployment descriptor contains:


<enterprisebean-client-jar>enterprisebean1_client.jar</enterprisebean-client-jar>

 enterprisebean1_client.jar

 enterprisebean2.jar Class-Path: enterprisebean1_client.jar
...
app3.ear:
 ...
 enterprisebean3.jar Class-Path: enterprisebean1_client.jar

 webapp.war Class-Path: enterprisebean1_client.jar
...
app1.ear:
 ...
 META-INF/enterprisebean-jar.xml
...
governed by the rules of the Jakarta language and the Java virtual machine.
...
and resources. Note that as of Java EE 6, Jakarta Enterprise Beans
components may be packaged in a web component module. Such Jakarta Enterprise Beans
...
. For each _enterprisebean-link_ , there must be only one
matching _enterprisebean-name_ in the scope of the entire application (see
link:#a936[See Jakarta™ Enterprise Bean References]”).
...
format files (for example, web and Jakarta Connector modules), all classes in
...
enterprise beans in an enterprisebean-jar file might be deployed multiple times
...
. If the _.jar_ file contains a
_META-INF/enterprisebean-jar.xml_ file, or contains any class with an Jakarta Enterprise Beans
...
example, the enterprise beans in an enterprisebean-jar file might be deployed
