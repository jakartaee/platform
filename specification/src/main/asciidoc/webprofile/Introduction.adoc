...
for quite some time, such as the Jakarta Enterprise Beans 4.x
...
similar reason, all Web Profile 9 products must support the Java(TM) Platform, Standard Edition 8 API,
and must run on a Java runtime defined as Java Platform, Standard Edition 11 Specification.
